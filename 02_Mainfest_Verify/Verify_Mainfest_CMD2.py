...
def put_result():
    ...
    print  fail_num
    if fail_num == 0:
         result_file.write(Manifest_Info.boardList[0][1]+': '+'Verify Finish! Result = PASS \n' )
    else:
            ...
            for exam_fold in example_fold:
                ...
                for exam_cate in os.listdir(example_fold_Path):
                    ...
                    if pattern.match(exam_cate):
                        ...
                        if exam_fold == 'sai' and  exam_type == 'driver_examples':   ## the  driver_examples/sai is special
                            
                            for sai_branch in os.listdir(example_fold_Path):
                                if os.path.isdir(example_fold_Path+'/'+sai_branch):
                                    example_name = sai_branch
                                    self.examplesPath = 'boards/'+board_path[0]+'/'\
                                                +exam_type+'/'+exam_fold+'/'+sai_branch   
                                    self.examples_category = exam_type+'/'+exam_fold
                                    for sai_name in os.listdir(example_fold_Path+'/'+sai_branch):
                                        example_name = sai_branch
                                        self.examplesPath = 'boards/'+board_path[0]+'/'\
                                                +exam_type+'/'+exam_fold+'/'+sai_branch   
                                        self.examples_category = exam_type+'/'+exam_fold
                                        if re.match('.*\.c$|.*\.h$',sai_name):
                                            self.CodeFiles.append(sai_name)
                                    self.CodeFiles.append('music.h')
                            ...
                            for exam_src in os.listdir(example_fold_Path) :    
                                ...
                                if os.path.isdir(example_fold_Path+'/'+exam_src) and exam_src != 'musci.h':
                                    for code_in_code in os.listdir(example_fold_Path+'/'+exam_src) :
                                         if pat.match(code_in_code):
                                            self.CodeFiles.append(code_in_code)
                            new_putle=(board_path[0],self.examples_category,exam_fold,self.CodeFiles[:],self.examplesPath)
                            self.example_codeList.append(new_putle)
                            del self.CodeFiles[:]
                            del self.examplesPath
                        ...
                        for exam_inner in os.listdir(exam_cate_Path):
                            ...
                            if pattern.match(exam_inner):
                                ...
                                for exam_src in os.listdir(exam_cate_Path) :    
                                    if pat.match(exam_src):
                                        self.CodeFiles.append(exam_src)
                                    if os.path.isdir(exam_cate_Path+'/'+exam_src) and exam_src != 'musci.h' :
                                        for code_in_code in os.listdir(exam_cate_Path+'/'+exam_src):
                                            if pat.match(code_in_code):
                                                self.CodeFiles.append(code_in_code)
                                if self.CodeFiles:    
                                ...
                                for exam_code in os.listdir(exam_inner_Path):
                                    ...
                                    if pattern.match(exam_code):
                                        ...
                                        for exam_src3th in os.listdir(exam_inner_Path) :    
                                            ...
                                            if os.path.isdir(exam_inner_Path+'/'+exam_src3th) and exam_src != 'musci.h':
                                                for code_in_code in os.listdir(exam_inner_Path+'/'+exam_src3th):
                                                    if pat.match(code_in_code):
                                                        self.CodeFiles.append(code_in_code)
                                        ...
                                        for exam_forth in os.listdir(exam_forth_path):
                                            ...
                                            if  pattern4.match(exam_forth):
                                                ...
                                                for exam_src4th in os.listdir(exam_forth_path) :    
                                                    ...
                                                    if os.path.isdir(exam_forth_path+'/'+exam_src4th) and exam_src != 'musci.h':
                                                        for code_in_code in os.listdir(exam_forth_path+'/'+exam_src4th):
                                                            if pat.match(code_in_code):
                                                                self.CodeFiles.append(code_in_code)
    ...
    def verify_XMLdriver_dupcheck(self):
        ...
        for k in range(len(componttype_List)):
            ...
            for filname in drvfliedict:
                ...
                if wrong_flag == 1:
                    wrong_flag = 0
                    result['drivers duplication '+ " --- manifest repeat in "+filname] = repeatlist
                    print repeatlist
                    repeatlist = []
            drvnameList = []
            drvflieList = []
    ...
    def mul_compare(self,listSDK,listXML,categoryname,examplename):
        ...
        if not diff1 and not diff2:
            ...
            if  diff1:
                ...
                result['examples code' + " --- manifest missing"+'in '+categoryname +':'+ examplename] = diff1
            if  diff2:
                print categoryname+examplename+'  vierify fail!\n',  diff2,' mainftes has more these files'
                result['examples code' + " --- manifest redundant"+' in '+categoryname +'-'+ examplename] = diff2
                return 1  ##notice!!! 
            return 0
