            ...
            if cmp(Last_AppParent, AppParent)!=0 and re.match(r'.*\.eww|.*\.cproject|.*\.uvprojx|.*\.bat.*',''.join(os.listdir(parent))):
                ...
                if not category:
                    for exam_src in os.listdir(AppParent):
                        if exam_src == 'startup' or exam_src == 'slcd_engine':
                            for exam_code in os.listdir(AppParent+'/'+exam_src):
                                if re.match(r'^.*\.c$|^.*\.h$',exam_code):
                                    source_list.append(exam_code)
                                
                        elif re.match(r'^.*\.c$|^.*\.h$',exam_src):
            ...
            for k in range(len(componttype_List)):
                ...
                for drvname in name_dupcheck:
                    if name_dupcheck[drvname] > 1:
                        print 'The driver '+ drvname +' of '+ componttype_List[k]+ ' is repeated'
                        repeatname_num += 1
                        self.fail_num += 1
                ...
                for filname in drvfliedict:
                    ...
                    for j in file_dupcheck:       
                        if file_dupcheck[j] > 1 :
                            print  'The drivers file '+ j +' in '+ filname + ' is repeated'
                            self.fail_num += 1
                            repeatname_num += 1
...
def put_result(result_path,result,fialnum):
    ...
    if fialnum == 0:
        ...
        print 'Verify Finish! Result = FAIL  '+str(fialnum)+' \n'
        result_file = open(result_path+'/Verify_Manifest_Failed.yml','a')
