...
root.title("Verify SDK mainfest")
...
def Read_Path_and_Run():
    ...
    if  os.path.exists(file_Path):
        if os.path.isdir(file_Path):
            ...
            ##
            valbar.set(100)
            print "Done! fail: " + str(fail_num) +' times'
        else:
...
def refresh_input():
    ...
    if run_flag == 1:
        ...
        fail_num = 0
        valbar.set(0)
        print " Please input a new SDK Path..."
    example_Input.set("")
...
##               if child.get('user_name') == "TWR_K80F150M" :  #boardList[int(boardSelection) - 1]="TWR-K21F120M"
                brdId = child.get('id')
...
class Verification:
    ...
    def verify_board_name(self):
        ...
        if self.classSDK.board_num == self.classXML.board_num and self.classXML.board_num ==1:
            ...
            print 'Result: this compare done, fail!'
    ...
    def verify_example_name(self):
        ...
        self.compare(example_nameSDK,example_nameXML)
        del example_nameSDK[:]
        del example_nameXML[:]
    ...
    def verify_example_category(self):
        ...
        self.compare(example_categorySDK,example_categorySDK)
    ...
    def  verify_driver_TYPE(self):
        ...
        self.compare(driver_typeSDK,driver_typeXML)
        del driver_typeSDK[:]
        del driver_typeXML[:]
    ...
    def compare(self,listSDK,listXML):
        global fail_num
...
##            print 'Result: the len is different between SDK and XML fail!'
##            fail_num += 1
            if self.longSDK < self.longXML:
...
Button(root, text='Refresh The SDK Path', bg='blue',fg='white',\
       font=('Arial', 10),command=refresh_input).pack(fill=X,side=TOP)

example_Input = StringVar()
example_Input.set("C:/01_MY_job/A_TO_DO/Verify_Mainfest/TWR-K21F120M_PKG_sdk_2_0_windows_iar")
Input=Entry(root,textvariable = example_Input,width=29,background='white',font=('Arial', 15),foreground='black')
Input.place(x=85,y=70)
Label(root,text='SDK Path:',font=('Arial', 10)).place(x=10,y=70)

valbar = IntVar() 
p = ttk.Progressbar(root, orient = "horizontal", length=200, mode="determinate", variable=valbar)
p.place(x=150,y=150) 

Run_Button=Button(root,text=' Run ',command=Read_Path_and_Run,font=('Arial', 10,'bold')).place(x=430,y=70)
